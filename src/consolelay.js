...
var _consoleLay = function(_config) {
  ...
  this.print = function() {
    ...
    if (self.config.closeBtn) {
      messageModal.closeBut = $(
        "<div class='arrow-console-close' title='close' ></div>"
      ).appendTo('body');
      var self = this;
  ...
  this.printPosition = function(self) {
    ...
    for (var i = 0; i < self.logArray.length; i++) {
      ...
      if (recorderSwitch) {
        totalHeight +=
          $(self.logArray[i].htmlElement).outerHeight() +
          self.option.messagePadding;
      }
    }
    var _self = self;
    (function(self) {
  ...
  this.closeItem = function(self, thisItem, that) {
    ...
    if (
      typeof self.logArray !== 'undefined' &&
      typeof self.logArray.length !== 'undefined'
    ) {
      var newArray = [];
